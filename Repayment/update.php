...
function updateRow(tableID) {
	...
	<?php
		...
		if ($Amount!=null){
			$results = mysqli_query($connect,"UPDATE `project`.`Repayment` SET `Amount` = '$Amount' WHERE `Repayment`.`Id` = '$myid' AND `Repayment`.`DateOfPayment` = '$DateOfPayment' ");
		}

		


		/*$results = mysqli_query($connect,"UPDATE `DB_Project`.`Repayment` SET `BId` = '$myid',`Name` = '$Name',`Town` = '$Town',`StreetName` = '$StreetName',`StreetNumber` = '$StreetNumber',`PostalCode` = '$PostalCode' WHERE `Repayment`.`BId` = '$myid' ");*/
