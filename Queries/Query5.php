...
<html>
	...
	<body>
		...
		<center><h2> The lender who has the highest Trust Percentage on a borrower </h2></center>
		<table id="Borrower">
		<?php
            		...
            		$results = mysqli_query($connect,"SELECT Lender.Name AS LN , Trust.Percentage AS TP
								FROM Trust , Lender , Borrower 
								WHERE Borrower.BId=Trust.BId AND Trust.LId = Lender.LId AND Trust.Percentage= (SELECT MAX(Trust.Percentage) FROM Trust) "); //nested
		?>
		<tr id="firstrow"><th>Lender's Name</th><th>Highest Trust Percentage</th></tr>			
		<?php
