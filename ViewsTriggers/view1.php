...
<html>
	...
	<body>
		...
		<?php
            		...
            		$results = mysqli_query($connect,"CREATE VIEW `Update`(Name,DateOfRequest,Percentage) 
								AS SELECT `Borrower`.`Name`,`LoanRequest`.`DateOfRequest`,`LoanRequest`.`Percentage`
								FROM `Borrower`,`LoanRequest`
								WHERE `Borrower`.`BId`=	`LoanRequest`.`BId` AND `LoanRequest`.`Amount` > 2000		"); //create the view

			$results = mysqli_query($connect,"SELECT * FROM `Update`"); //choose the view
		?>
		
		<tr id="firstrow"><th>Name</th><th>Date of Request</th><th>Percentage</th></tr>			
		<?php
